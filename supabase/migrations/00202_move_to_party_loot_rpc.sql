-- move_to_party_loot: Atomic move from character inventory/equipment to party loot.
-- Enforces ownership, quantity checks, and preserves full equipment metadata
-- (weapon/armor id, magic effects, bonuses) so items round-trip cleanly
-- when later distributed back to a character.
