-- give_character_gold_to_party: Atomic transfer from a character's personal
-- purse to the shared party loot pool. Enforces ownership and sufficient
-- funds so the UI can't be tricked into adding gold the character doesn't own.
...
BEGIN
  ...
  -- Atomic deduct from character with sufficient-funds guard
  UPDATE characters
  ...
  -- Add to shared party pool (singleton row)
